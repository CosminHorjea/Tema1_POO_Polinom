...
Polinom& Polinom::operator=(const Polinom& source) {
	...
	while (p) {// cat timp am noduri in source
...
void Polinom::insert(double val, unsigned int deg) {
	...
	*/
	...
	if (!size) {// daca am un polinom gol, start si end sunt la fel
		start = end = new_elem;
	}
	else {
		if (deg > start->grad) {//daca elem este primul
			new_elem->next = start;
			start = new_elem;//schimb start
			++size;
			return;// ma intorc
		}
		if (deg < end->grad) {//daca noul element este ultimul
			end->next = new_elem;
...
void Polinom::erase(unsigned int grad)
{
	...
	*/
	...
	if (size == 0)// daca Polinomul este gol
		throw - 1;// eroare
	if (size == 1) {// daca am doar un nod
		if (start->grad == grad) {//il sterg doar daca are gradul din paramtru
			delete start;
			start = end = 0;
			return;
		}
		else
			throw - 1;// altfel eroare
	}
	...
	while (aux)// cat timp am elemente in lista
	{
		if (aux->grad < grad) // daca am ajuns la un grad mai mic, sigur nu am gasit gradul deoarece elem sun sortate 
			break;
		if (aux->grad == grad) {// daca am gasit gradul cerut
			if (aux == end) {//daca gradul este ultimul in polinom
				prev = end;
				delete aux;
				return;
			}
	...
	throw - 1;// eroare daca nu am sters nimic
}
...
Polinom Polinom::operator+(Polinom& p2) const {
	...
	while (p && q) {// cat timp sunt elemente
		if (p->grad == q->grad) {// daca sunt egale gradele
		...
		else if (p->grad > q->grad) {// daca primul are grad mai mare
	...
	while (p) {// daca mi-au ramas elem in p le inserez
		if (p->coeff )
			result->insert(p->coeff, p->grad);
		p = p->next;
	}
	while (q) {// daca mi-au ramas elemente in q le inserez
		if (q->coeff)
			result->insert(q->coeff, q->grad);
		q = q->next;
	}
...
Polinom Polinom::operator*(Polinom& p2)const {
	...
	while (p) {// fiecare element din primul polinom
		Element* aux = q;
		while (aux) {// il inmultesc cu fiecare element din al doilea polinom
...
Polinom Polinom::operator*(const double scalar)const {
	...
	*/
	Polinom result;
	Element* p = start;
	while (p) {// fiecare element din Polinomul actual
...
Polinom Polinom::operator/(Polinom& p2)const {
	...
	Polinom result;
	while (deimpartit.start && (deimpartit.getDeg() >= p2.getDeg())) {// cat timp am deimpartit si gradul lui e mai mic deact al impartitorului
...
double& Polinom::operator[](unsigned int i)const {
	...
	*/
	...
	while (p) {// cat tim am element
		if (p->grad == i) {// daca gradul este cel din paramteru
